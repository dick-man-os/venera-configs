...
class EnReadtokyoghoulretokyoghoulmangaonlineSource extends ComicSource {
    ...
    comic = {
        loadInfo: async (id) => {
            ...
            let cover = EnReadtokyoghoulretokyoghoulmangaonlineSource.resolveAbsoluteUrl((doc.querySelector('div.flex > img') ? (doc.querySelector('div.flex > img').attributes['src'] || '') : ''), url);
            doc.dispose();
        ...
        loadEp: async (comicId, epId) => {
            ...
            let images = imgElements.map(el => EnReadtokyoghoulretokyoghoulmangaonlineSource.resolveAbsoluteUrl((el.attributes['data-src'] || ''), url)).filter(Boolean);
            doc.dispose();
    ...
    loadChapters = async (comicUrl) => {
        ...
        let chaptersList = elements.map(el => ({
            id: EnReadtokyoghoulretokyoghoulmangaonlineSource.resolveAbsoluteUrl((el.querySelector('.col-span-4 > a') ? (el.querySelector('.col-span-4 > a').attributes['href'] || '') : ''), url),
