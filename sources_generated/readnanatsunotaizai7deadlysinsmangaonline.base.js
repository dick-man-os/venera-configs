...
class EnReadnanatsunotaizai7deadlysinsmangaonlineSource extends ComicSource {
    ...
    comic = {
        loadInfo: async (id) => {
            ...
            let cover = EnReadnanatsunotaizai7deadlysinsmangaonlineSource.resolveAbsoluteUrl((doc.querySelector('div.flex > img') ? (doc.querySelector('div.flex > img').attributes['src'] || '') : ''), url);
            doc.dispose();
        ...
        loadEp: async (comicId, epId) => {
            ...
            let images = imgElements.map(el => EnReadnanatsunotaizai7deadlysinsmangaonlineSource.resolveAbsoluteUrl((el.attributes['data-src'] || ''), url)).filter(Boolean);
            doc.dispose();
    ...
    loadChapters = async (comicUrl) => {
        ...
        let chaptersList = elements.map(el => ({
            id: EnReadnanatsunotaizai7deadlysinsmangaonlineSource.resolveAbsoluteUrl((el.querySelector('.col-span-4 > a') ? (el.querySelector('.col-span-4 > a').attributes['href'] || '') : ''), url),
