...
class Keiyoushi7151191693036508367Source extends ComicSource {
    ...
    catalog = async (kind, keyword, page) => {
        ...
        if (!p || !Number.isInteger(p.page) || !Number.isSafeInteger(p.total_pages) || p.total_pages < 0 || p.page !== page)
            throw new Error("Invalid GlobalComix pagination");
        return {comics: this.unique(this.array(body.payload.results).map(this.manga)), hasMore: p.page < p.total_pages, maxPage: Math.max(1, p.total_pages)};
    };
    info = async id => {
