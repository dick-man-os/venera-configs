/**
 * @file en_readkingdommangaonline.base.js
 * Generated automatically by Venera Source Converter v0.1.0
 *
 * Upstream Project: keiyoushi
 * Upstream Package: eu.kanade.tachiyomi.extension.en.readkingdommangaonline
...
class EnReadkingdommangaonlineSource extends ComicSource {
    name = "Read Kingdom Manga Online"
    key = "en_readkingdommangaonline"
    version = "1.0.1"
    minAppVersion = "1.6.0"

    static baseUrl = "https://ww6.readkingdom.com"
    ...
    staticCatalog = [
        {
                "title": "Kingdom",
                "url": "https://ww6.readkingdom.com/manga/kingdom/"
        },
        {
                "title": "Li Mu",
                "url": "https://ww6.readkingdom.com/manga/li-mu/"
        },
        {
                "title": "Meng Wu & Chu Zi",
                "url": "https://ww6.readkingdom.com/manga/meng-wu-and-chu-zi-one-shot/"
        },
        {
                "title": "History Spoilers",
                "url": "https://ww6.readkingdom.com/manga/kingdom-history-spoilers/"
        }
];
    ...
    comic = {
        loadInfo: async (id) => {
            let url = id.startsWith("http") ? id : `${EnReadkingdommangaonlineSource.baseUrl}${id}`;
            let res = await Network.get(url, EnReadkingdommangaonlineSource.headers);
            if (res.status !== 200) {
            ...
            let cover = EnReadkingdommangaonlineSource.resolveAbsoluteUrl((doc.querySelector('div.flex > img') ? (doc.querySelector('div.flex > img').attributes['src'] || '') : ''), url);
            doc.dispose();
        ...
        loadEp: async (comicId, epId) => {
            let url = epId.startsWith("http") ? epId : `${EnReadkingdommangaonlineSource.baseUrl}${epId}`;
            let res = await Network.get(url, EnReadkingdommangaonlineSource.headers);
            if (res.status !== 200) {
            ...
            let images = imgElements.map(el => EnReadkingdommangaonlineSource.resolveAbsoluteUrl((el.attributes['data-src'] || ''), url)).filter(Boolean);
            doc.dispose();
        ...
        onImageLoad: (url, comicId, epId) => ({
            url: url,
            headers: {
                ...EnReadkingdommangaonlineSource.headers,
                "Referer": `${EnReadkingdommangaonlineSource.baseUrl}/`,
            },
        }),
    ...
    loadChapters = async (comicUrl) => {
        let url = comicUrl.startsWith("http") ? comicUrl : `${EnReadkingdommangaonlineSource.baseUrl}${comicUrl}`;
        let res = await Network.get(url, EnReadkingdommangaonlineSource.headers);
        if (res.status !== 200) {
        ...
        let chaptersList = elements.map(el => ({
            id: EnReadkingdommangaonlineSource.resolveAbsoluteUrl((el.querySelector('.col-span-4 > a') ? (el.querySelector('.col-span-4 > a').attributes['href'] || '') : ''), url),
